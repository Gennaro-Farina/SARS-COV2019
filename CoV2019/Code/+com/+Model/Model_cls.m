classdef Model_cls < matlab.mixin.Copyable
    ...
    methods(Static)
        ...
        function dydt = ODE_SEIR(~, y, beta, gamma, sigma)
         ...
         function dydt = ODE_SEIR_rho(~, y, beta, gamma, sigma, rho)
            S = y(1);
            E = y(2);    
            I = y(3);
            R = y(4);
            ...
            dydt    =  zeros(4,1);
            dydt(1) =  -(rho * beta*S*I)/N ;
            dydt(2) =  (rho * beta*S*I/N) - (sigma*E);    
            dydt(3) =  (sigma*E) - (gamma*I);
            dydt(4) =  gamma * I;
        end
        ...
        function [y, tout] = ode_solver_rho(tSpan, init_cond, beta, gamma, sigma, rho)
            import com.Model.*;
            ...
            opt = odeset('RelTol',1.0e-6,'AbsTol',1.0e-9, 'MaxOrder', 5);
            % non stiff solver               
            [tout, y] = ode45(@(t,y) Model_cls.ODE_SEIR_rho(t, y, beta, gamma, sigma, rho), ...
                        tSpan, y0, opt);
            
        end
        
         function [I]    = getI(params, tSpan, init_cond)
            import com.Model.*;
...
%             err = abs(A1 - A2);
        ...
        function [S, E, I, R] = getSEIRrho(params, tSpan, init_cond)
            import com.Model.*;

                        
            [beta, gamma, sigma, rho] = deal(params(1), params(2), params(3), params(4));
            
            sol = Model_cls.ode_solver_rho(tSpan, init_cond, beta, gamma, sigma, rho);
